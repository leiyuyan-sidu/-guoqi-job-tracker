...
每天自动抓取国聘（iguopin.com）的央企/国企校园招聘岗位，用规则+大模型判断哪些岗位符合"国际商务硕士、已过英语四六级"的报名条件，展示在一个静态网页上，可以标记"已投递/不投递"。
...
- `ANTHROPIC_API_KEY`：你的 Anthropic API key，去 [console.anthropic.com](https://console.anthropic.com) 申请，用于给规则判断不出来的岗位做语义匹配（用的是 Haiku 模型，成本很低）
